   ...
   #      pass
    ...
    def __forward_fundamental_rule(self, edge):
        ...
        for each_edge in succeeding_edges:
            ...
            c = self.__add_to_agenda(new_edge)
            if c:
                self.history.append({"original": edge, "generated": new_edge}) # NOT CALLED??? meaning this whole function
    ...
    def __backward_fundamental_rule(self, edge):
        ...
        for each_edge in precedent_edges:
            ...
            c = self.__add_to_agenda(new_edge)
            if c:
                self.history.append({"original": edge, "generated": new_edge})
    ...
    # スタック　＝＞　深さ優先探索
    # キュー　　＝＞　幅優先探索
    # returns 'True' if pushed to agenda, 'False' if there existed duplicate
